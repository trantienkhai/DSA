...
void StackFrame::run(string filename) {
    ...
    while (!fs.eof())
    {
        ...
        {
            if(avl.searchKey(string(result[1])) == nullptr){
                        free_string(result, num_words);
                        stack.clear();
                        avl.destroyTree();
                        fs.close();
                ...
                if(avl.searchKey(string(result[1]))->type == 0){
                    if(!stack.full()){
                        stack.push((int)avl.searchKey(string(result[1]))->data,0);
                    }
                    else{
                        free_string(result, num_words);
                        stack.clear();
        ...
        else if (string(result[0]) == "fload")
        {
             if(avl.searchKey(string(result[1])) == nullptr){
                ...
                if(avl.searchKey(string(result[1]))->type == 1){
                    if(!stack.full()){
                        stack.push((float)avl.searchKey(string(result[1]))->data,1);
                    }
                    else{
                        free_string(result, num_words);
                        stack.clear();
        ...
        else if (string(result[0]) == "istore")
        {
            ...
            if(stack.top_type() == 0)
            {
                if(avl.size() == 128){
                    ...
                    stack.pop();
                    if(avl.searchKey(string(result[1])) != nullptr){
                        avl.searchKey(string(result[1]))->data = (int)top_val;
                    }
                    else{
                    avl.insertNode(string(result[1]), (int)top_val, 0);
                    }
                }
            }
            else
           ...
           if(stack.empty()) {
            ...
            if(stack.top_type() == 1)
            {
                if(avl.size() == 128){
                    ...
                    stack.pop();
                    if(avl.searchKey(string(result[1])) != nullptr){
                        avl.searchKey(string(result[1]))->data = (float)top_val;
                    }
                    else{
                    avl.insertNode(string(result[1]), (float)top_val, 1);
                    }
                }
            }
            else
        ...
        else if (string(result[0]) == "val")
        {
            ...
            else
            {
                if (avl.searchKey(string(result[1]))->type == 0)
                {
                    cout << (int)avl.searchKey(string(result[1]))->data << "\n";
                }
                if (avl.searchKey(string(result[1]))->type == 1)
                {
                    cout << (float)avl.searchKey(string(result[1]))->data << "\n";
                }
               
            }
        }
        //lenh 33 par <var>
        else if (string(result[0]) == "par") {
            if (avl.searchKey(string(result[1])) == nullptr)
            {
                free_string(result, num_words);
                stack.clear();
            ...
            {
                if(avl.parent(string(result[1])) != nullptr){
                    cout << avl.parent(string(result[1])) -> key << "\n";
                }
                else {
                    cout << "null" << "\n";
                }
